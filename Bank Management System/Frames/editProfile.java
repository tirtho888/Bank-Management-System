...
public class editProfile extends JFrame implements ActionListener,MouseListener
{
	...
	JLabel name,fatherName,mail,dob,radbutton,dep,marital,city,username,pin,address;
	...
	public editProfile(user u,users us)
	{
		super("Sign Up page");
		...
		fatherText = new JTextField(u.getFatherName());
		...
		r2.setBounds(280,200,120,30);
		panel3.add(r2);
		r3 = new JRadioButton("Other");
		r3.setBounds(400,200,70,30);
		panel3.add(r3);
		...
		panel3.add(dob);
		dobField = new JTextField(u.getDob());
		...
		panel3.add(mail);
		mailtext = new JTextField(u.getMail());
		...
		panel3.add(username);
		userField = new JTextField(u.getUserName());
		...
		panel3.add(pin);
		pinField = new JPasswordField(u.getUserPass());
	...
	public void actionPerformed(ActionEvent ae){
		...
		if(ae.getSource()==back){
						dispose();
						UserProfile ftf = new UserProfile(u,us);
						ftf.setVisible(true);
					}
		...
		String radbutton = null;
					...
					if((!usname.isEmpty()) && (!radbutton.isEmpty()) && (!address.isEmpty())){
				...
				if(result == 0){
					String oldUserName=u.getName();
					String oldAddress=u.getAddress();
					String oldFatherName=u.getFatherName();
					String oldEmail=u.getMail();
					String oldName=u.getUserName();
					String oldPin=u.getUserPass();
					String oldDOB=u.getDob();
					String oldGender=u.getGender();
					String oldMarital=u.getMarital();
					user oldUser=new user(oldUserName,oldAddress,oldFatherName,oldEmail,oldName,oldPin,oldDOB,oldGender,oldMarital);
					...
					u.setUserPass(pin);
					u.setDob(dob);
					u.setGender(radbutton);
					u.setMarital(marital);
